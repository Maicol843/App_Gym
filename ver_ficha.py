...
class VerFicha(ctk.CTkFrame):
    def __init__(self, parent, id_cliente, callback_volver):
        ...
        ctk.CTkButton(header, text="← Volver", width=120, command=self.callback_volver).pack(side="left")
    ...
    def cargar_datos_y_mostrar(self):
        ...
        btns.pack(pady=30)
        ctk.CTkButton(btns, text="Editar", fg_color="#0d6efd", command=lambda: self.abrir_modal_edicion(c)).pack(side="left", padx=10)
        ctk.CTkButton(btns, text="Eliminar", fg_color="#dc3545", command=self.eliminar_cliente).pack(side="left", padx=10)

    def crear_bloque_info(self, parent, titulo, items):
        frame = ctk.CTkFrame(parent, fg_color="transparent")
        frame.pack(fill="x", padx=15, pady=10)
        ctk.CTkLabel(frame, text=titulo, font=("Arial", 14, "bold"), text_color="#0d6efd").pack(anchor="w")
    ...
    def abrir_modal_edicion(self, c):
        ...
        ctk.CTkButton(scr, text="Guardar", fg_color="#198754", height=40, command=guardar).pack(pady=20)
